...
class PortfolioStatsColumn extends StatelessWidget {
  const PortfolioStatsColumn({super.key, required this.tokens});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        MobileStatItem(label: 'Total Value', value: '\$${totalValue.toStringAsFixed(2)}', icon: Icons.attach_money),
        ...
        const SizedBox(height: AppSpacing.sm),
        MobileStatItem(label: 'Tokens', value: '${tokens.length}', icon: Icons.token),
      ],
    );
  }
}
